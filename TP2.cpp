 ...
 * iii)proporcion de particiones / cantNodos (en caso de haber elegido random)
...
int main(int argc, char **argv) {
  ...
  if(randomness == "notrandom") { 
    sprintf(ejes, "Resultados/ejes%s%s.out", argv[1], argv[2]);
    sprintf(labels, "Resultados/labels%s%s.out", argv[1], argv[2]);
    ...
    sprintf(ejes, "Resultados/ejes%s%s.out", argv[1], argv[3]);
    sprintf(labels, "Resultados/labels%s%s.out", argv[1], argv[3]);
  ...
  */
  ...
  for(int v1=0; v1<N; v1++) {
    for(int v2=v1+1; v2<N; v2++) {
      if (M[v1][v2] == 1) { streamEjes << v1+1 << " " << v2+1 << endl; }
    }
  } streamEjes.close();
