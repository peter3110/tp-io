...
\textit{Aclaracion:} (1) y (2) no son escalares sino que van a ser usados para referenciar a las inecuaciones en las demostraciones.

\label{modelo-pedro}

$Min \sum_{j=1}^{n} w_j$ sujeto a:
\begin{align*}
(1)\ \sum_{v_i \in V_p}\sum_{j=1}^{n} x_{ij} = 1 \qquad & \forall \ V_p \in \{V_1, \ldots, V_k\} \qquad & \text{//un nodo coloreado por particion}\\
(2)\ x_{ij} + x_{i'j} \leq 1 \qquad & \forall \ (i,i') \in E, \ \forall \ j = 1, \ldots, n \qquad & \text{//nodos adyacentes no tienen el mismo color}\\
k * w_j \geq \sum_{i=1}^{n} x_{ij} \qquad & \forall \ j \in \{1, \ldots, k\} \qquad & \text{//desigualdad color vs nodos con ese color}\\
w_j \leq \sum_{i=1}^{n} x_{ij} \qquad & \forall \ j \in \{1, \ldots, k\} \qquad & \text{//idem}\\
x_{ij} \in \{0,1\}\\
w_j \in \{0,1\}\\
\end{align*}
...
Este modelo es igual a Modelo Pedro, salvo que la ``desigualdad color vs nodos con ese color'' se modela de la siguiente forma:
