...
Este último modelo tiene más restricciones que el anterior, dado un mismo grafo. En Modelo Pedro, las últimas restricciones son $2 * P$ (donde P es la cantidad de particiones), mientras que en Modelo Santiago son $N * P$. Tener más restricciones puede llevar a que el tiempo de cómputo sea mayor. La comparación de estos modelos está en la sección \ref{comparacion-modelos}
...
Para implementar el algoritmo de CB, elegimos experimentalmente tres valores arbitrarios, CANT\_CICLOS\_CB, CANT\_RESTR\_CLIQUE y CANT\_RESTR\_AGUJERO, que determinan respectivamente cuántas veces buscamos un grupo de planos de corte para agregar al LP, una cota para la cantidad de cliques a agregar en cada iteración y una cota para la cantidad de agujeros agregados en cada iteración.
		...
		\For{ cada color posible }{
			\For{$i$ in $0..$ CANT\_RESTR\_CLIQUE }{
				\eIf{$ i==0 $}{
					...
					clique = dameClique(sol, modelo, color, nodoInicial = nodo random con peso $> 0$)
				}
				\If{clique todavia no fue agregada al modelo}{
					agregarRestriccionClique(modelo, clique)
				}
			}
			\For{$j$ in $0..$ CANT\_RESTR\_AGUJERO}{
				agujero = dameAgujero(sol, modelo, color) \\
				\If{agujero todavia no fue agregado al modelo}{
					agregarRestriccionAgujero(modelo, agujero)
				}
			}
		}
	}	
\caption{Algoritmo para agregar restricciones de tipo clique y tipo agujero}
\end{algorithm}


\begin{algorithm}[H]
	\SetKwInOut{Input}{input}
	\Input{solucion actual (sol) + modelo original relajado + conjunto de nodos $S$ inicial (con un unico nodo) que forman una clique}
	\While{todavia se puede agregar un nodo a la clique generada por $S$}{
...
\caption{Algoritmo para generar una clique que viole la desigualdad 1}
\end{algorithm}
...
\caption{Algoritmo para generar un agujero que viole la desigualdad 2}
\end{algorithm}
...
Modelo Pedro y Santiago sin randoms (algunos ejemplos en particular)

Agregando Randoms y mostrar que mejora (los mismos ejemplos de arriba para mostrar que mejora)

Pedro
	BB
		Variando recorrido arbol (comparando david, myciel, input con notrandom y random 0.2 0.4 0.8 1) -> compara recorrido arbol entre si en un grafico de barras
		Variando variable de corte (comparando david, myciel, input con notrandom y random 0.2 0.4 0.8 1) -> compara variable corte entre si en un grafico de barras
	CB
		idem BB
...
Comparaion modelos Pedro vs Santiago
